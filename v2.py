...
class LoginApp:
    ...
    def show_signup_form(self):
        ...
        def register_user():
            ...
            if cursor.fetchone() is not None:
                tk.messagebox.showerror("Error", "User with this email already exists!")
                conn.close()
                return
    ...
    def show_selection_window(self):
        ...
        selection_canvas.create_image(0, 0, image=self.bg_photo, anchor='nw')
        ...
        non_veg_hover_photo = ImageTk.PhotoImage(non_veg_hover_image)

        buttons_frame = tk.Frame(selection_canvas, bg='white')
        buttons_frame.place(relx=0.5, rely=0.5, anchor='center')

        def on_enter_veg(event):
            veg_button.config(image=veg_hover_photo, compound='center')

        def on_leave_veg(event):
            veg_button.config(image='', compound='center')

        def on_enter_non_veg(event):
            non_veg_button.config(image=non_veg_hover_photo, compound='center')

        def on_leave_non_veg(event):
            non_veg_button.config(image='', compound='center')

        veg_button = tk.Button(buttons_frame, text="Veg", font=("Helvetica", 18), width=15, height=2, bg="#e23744", fg="white", command=lambda: self.show_restaurants("Veg"))
        veg_button.grid(row=0, column=0, padx=20, pady=20)
        veg_button.bind("<Enter>", on_enter_veg)
        veg_button.bind("<Leave>", on_leave_veg)

        non_veg_button = tk.Button(buttons_frame, text="Non-Veg", font=("Helvetica", 18), width=15, height=2, bg="#e23744", fg="white", command=lambda: self.show_restaurants("Non-Veg"))
        non_veg_button.grid(row=0, column=1, padx=20, pady=20)
        non_veg_button.bind("<Enter>", on_enter_non_veg)
        non_veg_button.bind("<Leave>", on_leave_non_veg)

    def resize_selection_bg_image(self, event, canvas):
        new_width = event.width
        new_height = event.height
        resized_bg_image = self.bg_image.resize((new_width, new_height), Image.LANCZOS)
        self.bg_photo = ImageTk.PhotoImage(resized_bg_image)
    ...
    def show_restaurants(self, category):
        ...
        for restaurant in restaurants:
            restaurant_label = tk.Label(restaurant_window, text=restaurant[0])
